...
def CreateDatabase(cur):
    ...
    cur.execute("CREATE TABLE Beers(Beerid INT PRIMARY KEY, UserName TEXT, drank_on DATE, City TEXT, zipcode INT, Lat FLOAT, lon FLOAT, beer TEXT, image TEXT)")
...
token=np.loadtxt('token.txt',dtype=np.str)
